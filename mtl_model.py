...
from fp_model import MolFPEncoder
...
class Embed(nn.Module):
    def __init__(self, attn_head=4, output_dim=128, d_k=64, d_v=64, attn_layers=4, dropout=0.1, disw=1.5, device='cuda:0'):
        ...
        self.gnns = nn.ModuleList([GraphConv(65, output_dim) if i == 0 else GraphConv(output_dim, output_dim) for i in range(attn_layers)])
...
class Fox(nn.Module):
    def __init__(self, task='reg', tasks=1, attn_head=4, output_dim=128, d_k=64, d_v=64, attn_layers=4, D=16, dropout=0.1, disw=1.5, device='cuda:0',fp_type=None):
        ...
        self.reset_params()
        self.fp_rep = MolFPEncoder(output_dim, dropout, fp_type, device)
    ...
    def forward(self, data):
        ...
        gnn_out = self.proj(x_p)
        fp_out = self.fp_rep(data)

        out = torch.cat([gnn_out, fp_out], dim=1)
